...
description: Use whenever the user works on extracting structured job fields from client-uploaded photos — vision model integration, camera-based job creation, image preprocessing, vision prompt engineering, merging vision output with text extraction, fallback logic. Covers model tier selection, schema validation, cost guardrails, and the image-pipeline.
---

# Multimodal job extraction skill

## When to invoke

Any of: multimodal, vision, image extraction, camera job, photo-to-job, vision model, Gemini vision, GPT-4o vision, infer scope from image, infer materials from image, image preprocessing for LLM, vision prompt, vision schema.
...
| --------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------ |
| Image extraction                                    | **Flash tier** (currently Gemini 2.5 Flash; whichever fast multimodal is current) | **Pro tier** (Gemini 2.5 Pro / Claude Sonnet vision) | 90%+ accuracy on common AU marketplace jobs at <$0.001/image |
...
**Never use Opus / GPT-4 Turbo for this job.** Cost is 30–100× higher; accuracy gain on this domain is marginal. Reserved for nuanced dispute reasoning, not visual classification.
...
If invalid both passes → fall back to text-only extraction + flag for admin review
  ↓
Merge with text extraction (if client also typed a description)
  ↓
Return to mobile for client confirmation
```
...
2. **EXIF**:
   - For **job-creation photos** (the client's "here's what I want done"): **strip EXIF before sending to vision API**. Privacy-protective — don't leak GPS/timestamp/device to the LLM provider.
...
4. **Max 4 images per job**: hard limit. More images don't improve extraction quality and balloon cost. Mobile UI enforces.
...
const VisionExtractionSchema = z.object({
  ...
  notes: z.string().max(300).optional(), // anything ambiguous worth flagging to client
});
```
...
System prompt lives at `apps/api/src/modules/jobs/prompts/vision-extraction.system.v1.md` (versioned):

```
You are a job scoping assistant for an Australian peer-to-peer marketplace.

Given 1-4 photos a client has uploaded, extract structured information about the job they want done. Output STRICT JSON matching the schema provided.

Rules:
1. Read all images. Treat them as different views of the same job unless clearly distinct.
...
6. Confidence: 1.0 = certain; 0.5 = guessing; 0.0 = no idea. Calibrate honestly — over-confidence will cause bad offers.
...
Refuse and return category="unknown" + low confidence if images show: nudity, violence, illegal activity, or content unrelated to a real-world job. (Content Safety should have filtered these earlier — this is a defence-in-depth check.)
```
...
When the client uploads photos AND types a description, run both extractions independently, then merge:

| Field           | Source of truth                                                                       | Rationale                                                    |
| --------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `title`         | Text extraction                                                                       | Clients phrase their own titles better than vision can infer |
...
| `scope`         | Vision augments text — concatenate as "Client says: [text]. Photos suggest: [vision]" | Both useful for the tasker                                   |
| `materials`     | Union of both, deduplicated                                                           | Vision catches things clients forget to mention              |
| `durationHours` | Average of both, weighted by confidence                                               | Clients underestimate; vision is more realistic              |
| `riskClass`     | Max of both (more conservative wins)                                                  | Safety bias                                                  |
| `riskFactors`   | Union                                                                                 |                                                              |
| `budget`        | Client only (never AI-derived without explicit nudge UI)                              | Tax/legal implications                                       |

When client provides only photos: vision result is the canonical extraction. Mobile UI shows fields pre-filled, client confirms or edits before publish.

When client provides only text: vision pipeline doesn't run. Text extraction handles it.
...
2. **Always preprocess images first.** Resize + JPEG-85 + EXIF strip (job-creation only) BEFORE sending to LLM. Saves ~70% of vision-token cost.
...
5. **PII redaction for non-image context.** If you pass the client's typed description alongside the images (for joint extraction), redact PII via `PiiRedactionService.scrub()` first.
...
7. **Cost guardrail per job.** Max 2 vision calls per job posting (primary + fallback). Hard cap, enforced in `LlmService`. Exceeded calls return cached/text-only extraction.
...
- `apps/api/src/modules/jobs/vision-extraction.service.ts` — the orchestrator
- `apps/api/src/modules/jobs/prompts/vision-extraction.system.v1.md` — versioned system prompt
- `apps/api/src/modules/jobs/schemas/vision-extraction.schema.ts` — Zod schema
...
- `packages/prisma/schema.prisma` — `JobPhoto` (stores blobUrl + exifJson + moderation), `Job.extractedFields` (Json)
...
| --------------------------- | -------------------------- | --------------------------------------------------- |
| Per-job vision call cap     | 2                          | Hard limit — primary + 1 fallback                   |
...
- **Extraction accuracy** — % of vision extractions that the client accepts without editing critical fields (category, durationHours)
...
For each major prompt or model change, run against a hand-labelled set of ~50 job images covering:

- All 8 categories (6+ examples each)
- High / medium / low risk
- Indoor / outdoor / mixed
- Single image vs multi-image jobs
...
After backend extraction completes, mobile shows the prefilled job form with:
...
- **A confirmation step**: client must explicitly confirm before publish. Never auto-publish from vision extraction — always human-in-the-loop.
...
- Voice-driven job posting — different pipeline (speech-to-text → text extraction, not images)
