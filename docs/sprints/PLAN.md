...
| 2   | Mon 6 Jul → Fri 17 Jul  | **First user-visible sprint:** Mobile Auth + Onboarding (welcome carousel) + Client→Tasker upgrade backend + Stripe Connect + ABN/ABR (License module deferred to S4)                                                                   | ~95                                              | **First "click through the app" demo:** cold-launch → 3-screen welcome → email/Google/Apple signup against real backend → OTP entry → role select → client home → biometric re-login → "Become a tasker" CTA → Stripe Connect webview → ABN entry + ABR verify → held-funds banner                                                                                        |
| 3   | Mon 20 Jul → Fri 31 Jul | Job posting + AI extraction + **guest mode**                                                                                                                                                                                            | ~130                                             | **Guest** browses feed → posts a job with photos → AI auto-categorises + suggests budget → ReAct clarifying questions → "Sign up to publish" → fresh-user signup → job posts to new account                                                                                                                                                                               |
| 4   | Mon 3 Aug → Fri 14 Aug  | Discovery + Offering + Public Q&A + **License verification module + offer-time guard + expiry cron**                                                                                                                                    | ~121                                             | Tasker sees ranked feed + map → makes an offer → offer-time guard fires for unlicensed plumbing job → tasker uploads plumbing licence → admin (in admin console scaffold) approves → tasker can now offer → client sees offer list with "Verified Plumber" badge, accepts. Builder conditional rule demo'd by offering $4K (no licence needed) vs $7K (licence required). |
| 5   | Mon 17 Aug → Fri 28 Aug | Messaging + Payments core + **OTP swap to real provider**                                                                                                                                                                               | ~118                                             | Live chat between client + tasker → off-platform warning fires → card added → PaymentIntent + SetupIntent → mock OTP swapped for real provider before money flows                                                                                                                                                                                                         |
...
| 8   | Mon 28 Sep → Fri 9 Oct  | Notifications + Trust/Safety + Privacy                                                                                                                                                                                                  | ~85                                              | Push fires on offer/accept/dispute → EXIF flags suspicious photo → DSR export download → consent ledger                                                                                                                                                                                                                                                                   |
...
| 12  | Mon 23 Nov → Fri 4 Dec  | Soft launch + first real users + retrospective                                                                                                                                                                                          | ~50                                              | First real tasker takes a job in one Sydney suburb, end-to-end with real money (test mode → live)                                                                                                                                                                                                                                                                         |

**Total: ~1098 hours** across Sprints 1-12 (estimate v1.1 was 945; +20h Sprint 3 guest-mode + deferred-auth, +13h Sprints 2 + 11 onboarding screens, +5h Sprint 5 OTP swap pattern, +21h Sprint 4 License module + offer-time guard + expiry cron per ADR 005, +94h net from the rebalance (Sprint 1 trimmed to backend-only at 80h, Sprint 2 grew to 95h absorbing mobile auth + tasker backend). Sprint 0 is foundation overhead and not counted vs the v1.1 budget.
