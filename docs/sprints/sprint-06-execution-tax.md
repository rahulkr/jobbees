...
**Hours budget:** ~120 (63 mobile, 84 backend, 8 Flutter Web parity; +16 per 14 Jun Estimation v1.2 verification — M-233 +2, B-54 +6, B-55 +8; +1 row 149 per audit)
...
| 149   | Cancellation history per task (per Estimation v1.2 audit)                                                                                                                                                                                 | THIN | 1   | Simplified log of cancellations on a task.               |
| M-233 | **Annual income statement download** (per Estimation v1.2 verification 14 Jun) — mobile screen lists available income statements by financial year. Tap to download PDF generated by B-55 batch job. Required for contractor tax returns. | IN   | 2   | MDR §5.2. ATO requirement. UI is simple list + download. |

**Mobile total: ~63h** (+2h M-233, +1h row 149 per 14 Jun Estimation v1.2 verification + audit)
...
| B-54 | **TFN encryption at rest (AES + Azure Key Vault)** (per Estimation v1.2 verification 14 Jun) — Privacy Act + ATO requirement. TFN stored AES-encrypted via Azure Key Vault. Controlled-decrypt path only — TFN never logged, never returned in API responses. Migration job for any existing unencrypted records.      | IN   | 6   | MDR §5.2. Highest-sensitivity field on the platform. Tax advisor + security review skill mandatory. |
| B-55 | **Annual income statement batch job (July cron)** (per Estimation v1.2 verification 14 Jun) — runs 1 July each year. Aggregates all completed task earnings per contractor for the financial year. Generates PDF stored in Azure Blob. Download link emailed via SendGrid. Mobile screen M-233 provides in-app access. | IN   | 8   | MDR §5.2. ATO mandatory. Infrastructure must be ready at launch even though first run is July 2027. |

**Backend total: ~84h** (+14h per 14 Jun Estimation v1.2 verification: B-54 TFN encryption +6, B-55 income statement batch job +8)

### Flutter Web parity (added per founder direction 14 Jun 2026)

| ID    | Item                                                                                                                                 | Call | Hrs | Notes                                                                                                                                             |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------ | ---- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| FW-19 | **Web parity — job tracker view (client follows tasker progress), completion proof viewer, tax invoice PDF viewer, RCTI PDF viewer** | IN   | 5   | Geofence check-in + live location are mobile-natural (no equivalent on web — desktop geolocation is unreliable). Web shows the read-only tracker. |
| FW-20 | **Web parity — cancellation flow (request, confirm, fee preview, dispute trigger)**                                                  | IN   | 3   |                                                                                                                                                   |
