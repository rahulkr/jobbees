...
> **Update (client note #4, PR#50):** Signup is simplified — **everyone signs up as a client; there is no role picker at signup.** Becoming a tasker is an in-app upgrade from the **profile screen** (row 15), which already triggers Stripe Connect + ABN. The web "Work with us" direct-tasker signup is deferred to the web sprint. Row 14 is re-scoped accordingly (below); the backend is unchanged (it already defaults `CLIENT` when no role is sent).
...
**Hours budget:** ~140 (67 mobile, 30 backend, 5 admin scaffolding, 11 cross-cutting from scope reconciliation, 20 Flutter Web parity, 7 per 14 Jun Estimation v1.2 verification, +7 per Estimation v1.2 final audit — rows 26/27/32/50/51)
**Mid-sprint demo:** Fri 10 Jul
**End-of-sprint demo:** Fri 17 Jul

## Goal in one sentence

By Friday 17 Jul, a user can cold-launch the app, see the welcome carousel, sign up (email/Google/Apple) as a client, verify OTP (against MockOtpService), land on client home, open their profile and tap "Become a tasker", complete Stripe Connect onboarding (Stripe handles identity KYC), verify their ABN — all against the real Sprint 1 backend, running entirely on local Docker.
...
| 2   | Email signup (first name, last name, email, mobile, location)         | IN   | 4   | Single form with validation; hits Sprint 1 `/auth/signup`. No user-type/role field — everyone signs up as a client (client note #4).                                                                                |
...
| 14  | Signup role handling — everyone signs up as a client (no role picker) | IN   | 2   | Client note #4 (PR#50): removed the Client/Tasker/Decide-later picker. Becoming a tasker is an in-app upgrade from the profile screen (row 15). Web "Work with us" direct-tasker signup deferred to the web sprint. |
...
| 27  | KYC re-submission flow (per Estimation v1.2 audit)                    | IN   | 2   | Guides rejected tasker through re-upload with reason shown.                                                                                                                                                         |
| 32  | Address book — saved addresses (per Estimation v1.2 audit)            | THIN | 2   | Save + reuse common task locations.                                                                                                                                                                                 |
| 50  | Completion stats / badges (per Estimation v1.2 audit)                 | THIN | 1   | Tasks completed + completion rate. Renders on public tasker profile.                                                                                                                                                |
| 51  | Tasker reputation score display (per Estimation v1.2 audit)           | THIN | 1   | `trust_score` rendered on public profile; computed in S7.                                                                                                                                                           |
...
### Cross-cutting (added per scope reconciliation, closes gaps #9, #19, #20, #23)

| ID     | Item                                                                                                                                                                                                                                                                                                                                                                                 | Call | Hrs | Notes                                                                                                                                                                                          |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 217    | **Product analytics — PostHog SDK + event taxonomy** — `posthog_flutter` package on mobile, `posthog-node` on backend, `posthog-js` on admin web. Single `track()` wrapper. Define MVP event names (`signup_started`, `signup_completed`, `connect_started`, `connect_completed`, etc.).                                                                                             | IN   | 4   | Instrument as we build — skipping = no historical funnel data forever. Self-hosted PostHog small tier or PostHog Cloud free tier. PII-safe (no email/phone in event props; use `userId` only). |
| 216    | **Sentry crash reporting** — `sentry_flutter` on mobile, `@sentry/nextjs` on admin web. DSN in env. Tag releases with the git SHA.                                                                                                                                                                                                                                                   | IN   | 2   | Free tier covers MVP volume. Source maps uploaded in CI. Backend already has App Insights so Sentry is mobile + admin only.                                                                    |
| 206    | **Deep linking — Universal Links (iOS) + App Links (Android)** — apple-app-site-association + assetlinks.json hosted on the public web origin; route handler in `go_router`. Cover `/job/:id`, `/share/:token`, `/notification-tap/:id`.                                                                                                                                             | IN   | 3   | Needed for share links from Sprint 3 onwards and notification taps from Sprint 8. Cheaper to set up now than retrofit.                                                                         |
| 218    | **Mobile a11y foundations (WCAG 2.1 AA targets)** — semantic widgets on every Material widget, Tooltip on icon buttons, focus order check, contrast verified against `apps/mobile/lib/theme/`. Add `flutter_lints` accessibility-related rules. Hook a TalkBack/VoiceOver pass into Sprint 11 audit.                                                                                 | IN   | 2   | Foundations only this sprint — full audit at Sprint 11. Cheaper to bake in as we build than retrofit.                                                                                          |
| M-228  | **AI Welcome Agent (onboarding walkthrough)** (per Estimation v1.2 verification 14 Jun) — conversational agent on first login. Different journeys for tasker (5-min skill setup) vs client (post-your-first-job). Claude Haiku. Skippable. Lands on home after exit.                                                                                                                 | IN   | 4   | Blueprint §18. Skill from skills/multimodal-extraction not needed here — text-only Haiku. Survives offline by caching the script. AuditLog the skip rate.                                      |
| DEV-12 | **Sentry PII-scrubbing rules** (per Estimation v1.2 verification 14 Jun — enhances the Sentry SDK wired earlier) — explicit redaction list in Sentry config: email, phone, name, ABN, TFN, license number, bank details all stripped from error payloads before transmission. Source maps uploaded for readable stack traces. Alert routing: critical → PagerDuty, warnings → Slack. | IN   | 3   | Privacy Act compliance — cannot ship to production without it. Stacks with the Sentry SDK wired earlier in this sprint.                                                                        |

**Cross-cutting total: ~11h**

### Flutter Web parity (added per founder direction 14 Jun 2026)

Web build target enabled in S1; this sprint adds web parity for every user-visible mobile screen built above. One Flutter codebase, two surfaces.

| ID    | Item                                                                                                                  | Call | Hrs | Notes                                                                                                                                                                                                                                                                                                                                        |
| ----- | --------------------------------------------------------------------------------------------------------------------- | ---- | --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FW-04 | **Web parity — welcome carousel, signup, OAuth, login, biometric fallback, OTP, suspended/banned screens**            | IN   | 6   | Responsive breakpoints (mobile / tablet / desktop), web-safe biometric fallback (WebAuthn passkey where available, else password-only with link-via-email), OAuth via redirect flow vs popover. Web also signs everyone up as a client (no role picker), matching mobile; the "Work with us" direct-tasker entry is a later web-sprint item. |
| FW-05 | **Web parity — client profile setup + edit + public profile view + verification badges**                              | IN   | 3   |                                                                                                                                                                                                                                                                                                                                              |
| FW-06 | **Web parity — Client→Tasker upgrade flow + ABN entry UI + verification status**                                      | IN   | 3   | Same backend endpoints; web surface for the upgrade trigger.                                                                                                                                                                                                                                                                                 |
| FW-07 | **Web parity — Stripe Connect onboarding via in-page Stripe.js (not webview — web uses Stripe-hosted redirect flow)** | IN   | 4   | Stripe Connect Express onboarding is fully web-native. Redirect to Stripe → return to our success URL with state.                                                                                                                                                                                                                            |
| FW-08 | **Web parity — tasker profile wizard, public tasker profile, portfolio, reviews placeholder, held-funds banner**      | IN   | 4   |                                                                                                                                                                                                                                                                                                                                              |
...
00:40 — Signup screen. Pick "Sign up with email". Fill form (Rahul Test,
        rahul@example.com, +61400000000, Sydney). Submit. Everyone signs
        up as a client — no role picker.
...
01:20 — Email verification: tap simulated link. Land on client home
        (empty state — "No jobs yet. Posting comes in Sprint 3.").
...
| Wed 8 (D3)   | Google + Apple signup. Forgot password.                                                                                                                                                                       | Connect onboarding status tracking. Client→Tasker upgrade endpoint.                          |
...
| Thu 16 (D9)  | Tasker wizard: hourly rate + photo + portfolio scaffold. Held-funds banner + reminder cadence rendering. Public tasker profile. PostHog SDK wired + first 8 events firing. Sentry SDK wired (mobile + admin). | Polish. AuditLog write coverage check. `posthog-node` wired into backend with PII redaction. |
| Fri 17 (D10) | End-of-sprint demo + CSV update. Universal/App Links validated via Apple Validator + Android `adb shell pm verify-app-links`. Mobile a11y foundation pass (semantic widgets + contrast).                      | Confirm CI green. Tag `sprint-02-end`.                                                       |
...
- `feat(mobile): OTP + email verify + permissions priming`
- `feat(mobile): client profile + suspend/banned/deletion screens`
- `feat(mobile): universal signup (default client) + profile screen with tasker upgrade (client note #4, PR#50)`
