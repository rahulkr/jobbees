...
**Hours budget:** ~91 (75 backend, 5 admin gate, 9 Flutter Web foundation per 14 Jun founder re-scope, 2 account lockout per 14 Jun Estimation v1.2 verification)
...
| M-231 | **Account lockout after failed OTP / auth attempts** (per Estimation v1.2 verification 14 Jun) — 5 failed OTPs/hour → 1h lockout; 10 failed passwords → forced password reset via email. Redis counters. Cleared on successful auth. AuditLog write on every lockout. | IN   | 2   | Blueprint §4 / SRS NFR-SEC. Closes a specific gap in our auth surface.                                                                          |
...
| —     | AuditLog DB-level immutability — Prisma migration that REVOKEs UPDATE + DELETE on `"AuditLog"` from the app DB role (closes scope-reconciliation gap #27, AP-46/52)                                                                                                   | IN   | 1   | Belt-and-braces with the app-layer rule (security-review skill §I2). Test asserts UPDATE / DELETE from the app role raises `permission denied`. |
...
**Backend total: ~75h** (was 74h; +1h from the AuditLog DB-immutability migration added per scope reconciliation)
...
### Flutter Web target — foundation enablement (added per founder direction 14 Jun 2026)

Flutter Web is now in MVP scope, with full screen parity (mobile + web from one codebase) building out across Sprints 2-8. This sprint lays the foundation so the per-sprint web parity rows can land cleanly later.

| ID    | Item                                                                                                                                                                                                                                                                                                            | Call | Hrs | Notes                                                                                                                                                                                                                 |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FW-01 | **Flutter Web build target enabled + responsive base** — `flutter create --platforms=web` already done; this sprint adds the responsive layout primitives (`LayoutBuilder` breakpoints, web-safe `Platform.is*` guards, Cupertino/Material adaptive selection). Hot-restart works on Chrome.                    | IN   | 3   | Required before any web screen parity row can ship.                                                                                                                                                                   |
| FW-02 | **Web auth — HttpOnly cookie + CSRF flow** (per ADR 006) — backend auth surface already cookie-aware from this sprint's auth code; mobile uses Bearer in Authorization header, web uses `Set-Cookie` + `XSRF-TOKEN` double-submit. Same `/auth/*` endpoints; the surface is selected by the `X-Surface` header. | IN   | 4   | ADR 006 already specified this dual auth pattern. Backend implements both; per-surface dispatch is one if-branch on the surface header. Closes scope-reconciliation (founder re-scope) item — Flutter Web is now MVP. |
| FW-03 | **Web routing — `go_router` web URL strategy + deep-link routes** — `setUrlStrategy(PathUrlStrategy())`, web-friendly URL paths (`/jobs/:id`, `/post`, `/me`, `/auth/signup`), browser back/forward behaviour. Mirrors the deep-link surface added in Sprint 2 mobile cross-cutting.                            | IN   | 2   |                                                                                                                                                                                                                       |

**Flutter Web total: ~9h**
