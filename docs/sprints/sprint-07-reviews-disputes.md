...
**Hours budget:** ~119 (40 mobile, 69 backend, 8 Flutter Web parity per 14 Jun founder re-scope; +16h per 14 Jun Estimation v1.2 verification — B-48 fraud scoring +10, B-57 auto-suspend rules +6)
...
| B-48 | **Behavioural fraud scoring** (re-scoped IN per 14 Jun Estimation v1.2 verification — was V2) — detects synthetic identity, collusion patterns, platform-leakage signals. Flags taskers with frequent cancellations + abnormal velocity. Surfaced in admin Trust & Safety queue. Output `fraud_score` 0-1 stored on User; feeds B-57 auto-suspend rules engine.                                                        | IN★  | 10  | Estimate v1.2 marks IN★. Pairs with M-229 device fingerprinting (Sprint 8). Disputes sprint is the natural home — same data signals (cancellations, disputes lost, abnormal patterns). |
| B-57 | **Auto-suspend trigger rules engine** (per 14 Jun Estimation v1.2 verification) — BullMQ job evaluates thresholds after each dispute resolution, no-show, review reveal, and fraud-score update. Triggers: 3 disputes lost, 3 no-shows, rating <3.0 with ≥10 reviews, fraud_score >0.7. Auto-sets account suspended + logs reason code. Notifies tasker via push + email. Feeds reinstatement queue (AP-58, Sprint 9). | IN   | 6   | MDR §5.5. Disputes sprint is the natural home — most triggers fire on dispute outcomes.                                                                                                |
| B-41 | **Review authenticity scoring (LLM classifier)** (per Estimation v1.2 CHANGED-ITEMS confirm — IN★) — detects collusion patterns, GPT-phrasing anomalies, burst patterns. AI router (AI-01, S3) already built — this is one classifier endpoint + prompt engineering. Flagged reviews surface in Sprint 9 review moderation queue.                                                                                      | IN★  | 5   | Marked IN★ in Estimation v1.2. Reuses B-23 classifier pattern. Complements (doesn't replace) the manual review-removal flow.                                                           |

**Backend total: ~74h** (+16h per 14 Jun Estimation v1.2 verification: B-48 +10, B-57 +6; +5h B-41 review authenticity scoring per CHANGED-ITEMS confirm)

### Flutter Web parity (added per founder direction 14 Jun 2026)

| ID    | Item                                                                                                                         | Call | Hrs | Notes                                                            |
| ----- | ---------------------------------------------------------------------------------------------------------------------------- | ---- | --- | ---------------------------------------------------------------- |
| FW-21 | **Web parity — review submission, blind reveal, response, public review viewer**                                             | IN   | 4   | Review surfaces are well-suited to web (longer-form text input). |
| FW-22 | **Web parity — dispute opening, evidence upload (drag-drop on web), Tier-0 proposal viewer, accept/reject, escalation flow** | IN   | 4   | Drag-drop evidence upload on web is actually nicer than mobile.  |
