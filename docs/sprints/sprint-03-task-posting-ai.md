# Sprint 3 — Task posting + AI extraction + guest mode

**Dates:** Mon 20 Jul → Fri 31 Jul 2026 (10 working days)
**Theme:** Anyone — logged in OR a curious visitor — can post a task. They type a sentence, take a photo, the AI does the rest, and only at the moment of publish do we ask them to sign up.
...
By Friday 31 Jul, a **guest user** (no account) can open the app, browse tasks in their area, tap "Post a task", take a photo of a broken fence, see the AI extract a structured draft, tap Publish — and only then get prompted to sign up. The draft survives the signup and posts to their fresh account.
...
| 523 | **Guest mode — browse home feed without account**       | IN   | 5   | New: route group, read-only chrome, hide bid CTAs                                  |
...
| 525 | **Public read endpoints (anonymous browse)**                       | IN   | 4   | New: GET /v1/public/tasks with sensitive-field filter; rate-limited per IP                                  |
| 526 | **"Claim draft on signup" — attach pre-auth draft to new account** | IN   | 5   | New: signup endpoint accepts optional draft payload, creates Task with status DRAFT and posterId = new user |
...
- **Guest-mode draft handling: no new model needed.** Mobile keeps the draft in local storage (Hive / shared_preferences) until signup. On signup, mobile POSTs the draft body to the signup endpoint, which creates a Task with `status: DRAFT` + `posterId = newUserId`. Trade-off: draft is lost if the user uninstalls the app before signing up. Acceptable at MVP — user just retypes.
...
- Open task detail pages (read-only)
...
- Place a bid (CTA shows "Sign up to bid")
- View other users' profiles (returns 401 → "Sign up to view tasker profile")
- Use messaging
- See poster contact info
- Access /me/\* endpoints

**Public read endpoint design (`/v1/public/tasks`):**

- Returns task list with: title, description (first 200 chars), category, budget range, suburb (not full address), photo thumbnails, posted-at, status
- Filters out: full address, poster email/phone, poster full name (only first name + initial)
...
6. Backend creates Task with `posterId = newUserId` and `status = PUBLISHED`
7. User lands on the new task's detail page (their first task — celebratory state)
...
00:25 — Browse the ranked feed as guest. Tap a few tasks to see details.
        Notice the "Sign up to bid" CTA replaces the bid button.
00:45 — Tap "Post a task" floating action. Choice screen: type / photo / voice.
...
03:05 — **Bottom sheet appears: "Sign up to publish your task"**.
        Show the user is still anonymous. Pick "Continue with Email".
03:25 — Fill in name, email, mobile. Submit. OTP verify.
03:50 — App returns to the publish flow. Backend creates the task
        attached to the new account. Task lands in "My tasks" with
        status: PUBLISHED. Celebratory state ("Your first task is live!").
...
05:00 — Show admin view: tasks list with new tasks appearing, and the
...
| Day          | Mobile                                                                                | Backend                                                                                    |
| ------------ | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Mon 20 (D1)  | Guest-mode route group + read-only chrome. Posting entry point.                       | Task CRUD endpoints + state machine. Public read endpoint scaffold.                        |
| Tue 21 (D2)  | Category picker + text path. Guest "Sign up to bid" CTAs.                             | AI extraction (Gemini Flash JSON-mode). Prompt v1. Public read endpoint with field filter. |
...
| Thu 30 (D9)  | Save as draft + resume + edit + cancel. Polish.                                       | Task draft persistence. Audit log. Polish.                                                 |
...
- [ ] **Guest cold-launch → browse → post → AI extraction → "Sign up to publish" → fresh-user signup → task lands attached to new account, end-to-end**
- [ ] **Public read endpoint test: anonymous GET returns sanitised task list, never leaks full addresses or PII (skill §F4)**
...
- `feat(api/tasks): geocoding (Google Maps Platform — server-side proxy)`
...
- **`feat(api/tasks): public read endpoint + field-filter + rate-limit (anonymous browse)`**
- **`feat(api/auth): claim-draft-on-signup — accept optional draft payload, attach to new user`**
- `feat(mobile): guest-mode route group + read-only chrome + "Sign up to bid" CTAs`
