...
describe('UsersService.switchToClient', () => {
  it('switches a tasker back to a client and writes an audit row', async () => {
    const { service, prisma, audit } = build({
      id: 'u1',
      role: UserRole.TASKER,
    });

    const result = await service.switchToClient('u1', { ipAddress: '1.2.3.4' });
  ...
  it('is an idempotent no-op for an existing client', async () => {
    const { service, prisma, audit } = build({
      id: 'u1',
      role: UserRole.CLIENT,
    });

    const result = await service.switchToClient('u1');
  ...
  it('refuses to re-role an admin', async () => {
    ...
    await expect(service.switchToClient('a1')).rejects.toBeInstanceOf(ForbiddenException);
    expect(prisma.user.update).not.toHaveBeenCalled();
    expect(audit.record).not.toHaveBeenCalled();
  });
  ...
  it('throws when the user does not exist', async () => {
    ...
    await expect(service.switchToClient('ghost')).rejects.toBeInstanceOf(NotFoundException);
  });
});
